...
import { decodeOAuthState, exchangeCodeForToken, getInstagramAccountInfo } from '@/lib/oauth'

/**
 * GET /api/auth/instagram/customers/callback
...
export async function GET(request: NextRequest) {
  try {
    ...
    const accountInfo = await getInstagramAccountInfo(tokenData.accessToken, tokenData.userId)
    if (!accountInfo) {
      const redirectUrl = new URL('/dashboard', process.env.NEXT_PUBLIC_APP_URL || 'https://chatflow-ai-black.vercel.app')
      redirectUrl.searchParams.set('error', 'Failed to retrieve account information')
      return NextResponse.redirect(redirectUrl)
    }
    ...
    const { data: connectedPlatform, error: insertError } = await supabase
      ...
      .insert({
        ...
        platform: 'instagram',
        platform_account_id: accountInfo.pageId,
        account_name: accountInfo.accountName,
        access_token: tokenData.accessToken,
        webhook_secret: null, // Not needed for per-customer accounts
        status: 'active',
        ...
        metadata: {
          ...
          profile_picture_url: accountInfo.profilePic,
        },
      } as any)
      .select('id')
      .single()
    ...
    const redirectUrl = new URL('/dashboard', process.env.NEXT_PUBLIC_APP_URL || 'https://chatflow-ai-black.vercel.app')
    redirectUrl.searchParams.set('connected', `Instagram (${accountInfo.accountName})`)
    return NextResponse.redirect(redirectUrl)
  } catch (error) {
